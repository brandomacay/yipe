...
public class CollectionUtils {
    ...
    public static Streams collectStreamInfo(StreamInfo info) {
        ...
        info.getStreamSegments().forEach(segment -> chapters.add(new ChapterSegment(segment.getTitle(), URLUtils.rewriteURL(segment.getPreviewUrl()),
        ...
        info.getSubtitles()
                .forEach(subtitle -> subtitles.add(new Subtitle(URLUtils.rewriteURL(subtitle.getContent()),
        ...
        if (!livestream) {
            /*info.getVideoOnlyStreams().forEach(stream -> videoStreams.add(new PipedStream(stream.getItag(),
                    stream.getContent(),
...
*/
            info.getVideoStreams().forEach(stream -> videoStreams.add(new PipedStream(stream.getItag(),
                    stream.getContent(), String.valueOf(stream.getFormat()),
                    stream.getResolution(), stream.getFormat().getMimeType(), false,
                    stream.getItagItem().getContentLength())));


            /*info.getAudioStreams()
                    .forEach(stream -> audioStreams.add(new PipedStream(stream.getItag(), stream.getContent(),
        ...
        return new Streams(info.getName(), info.getDescription().getContent(),
                info.getTextualUploadDate(), info.getUploaderName(), URLUtils.substringYouTube(info.getUploaderUrl()),
                URLUtils.getLastThumbnail(info.getUploaderAvatars()), URLUtils.getLastThumbnail(info.getThumbnails()), info.getDuration(),
                info.getViewCount(), info.getLikeCount(), info.getDislikeCount(), info.getUploaderSubscriberCount(),
                info.isUploaderVerified(),
                audioStreams, videoStreams, relatedStreams, subtitles, livestream, info.getHlsUrl(),
                info.getDashMpdUrl(), null, info.getCategory(), info.getLicence(),
    ...
    private static StreamItem collectRelatedStream(Object o) {
        ...
        return new StreamItem(URLUtils.substringYouTube(item.getUrl()), item.getName(),
                URLUtils.getLastThumbnail(item.getThumbnails()),
                item.getUploaderName(), item.getUploaderUrl(),
                URLUtils.getLastThumbnail(item.getUploaderAvatars()), item.getTextualUploadDate(),
    ...
    private static PlaylistItem collectRelatedPlaylist(Object o) {
        ...
        return new PlaylistItem(URLUtils.substringYouTube(item.getUrl()), item.getName(),
                URLUtils.getLastThumbnail(item.getThumbnails()),
                item.getUploaderName(), item.getUploaderUrl(),
    ...
    private static ChannelItem collectRelatedChannel(Object o) {
        ...
        return new ChannelItem(URLUtils.substringYouTube(item.getUrl()), item.getName(),
                URLUtils.getLastThumbnail(item.getThumbnails()),
