...
import com.yipe.piped.utils.resp.SimpleErrorMessage;
...
public class SponsorBlockUtils {
    ...
    public static String getSponsors(String id, String categories, String actionType)
        ...
        for (String apiUrl : Constants.SPONSORBLOCK_SERVERS) {
            try {
                ...
                if (resp.status() == 200) {
                    var any = Constants.mapper.readTree(resp.body());

                    for (var element : any) {
                        if (element.get("videoID").asText().equalsIgnoreCase(id)) {
                            return Constants.mapper.writeValueAsString(element);
                        }
                    }
    ...
    public static CompletableFuture<ObjectNode> getDeArrowedInfo(String[] videoIds) {
        ...
        var futures = Arrays.stream(videoIds)
                .map(id -> getDeArrowedInfo(id, Constants.SPONSORBLOCK_SERVERS.toArray(new String[0]))
    ...
    static {
        EMPTY_DEARROWED_INFO = Constants.mapper.createObjectNode();
