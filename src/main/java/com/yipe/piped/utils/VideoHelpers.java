...
public class VideoHelpers {
    public static void handleNewVideo(String url, long time, Channel channel) {
        try {
            var extractor = Constants.YOUTUBE_SERVICE.getStreamExtractor(url);
            extractor.fetchPage();
    ...
    public static void handleNewVideo(StreamInfo info, long time, Channel channel) throws Exception {
            ...
            try (StatelessSession s = DatabaseSessionFactory.createStatelessSession()) {
                if (!DatabaseHelper.doesVideoExist(s, info.getId())) {
                    ...
                    Video video = new Video(info.getId(), info.getName(), info.getViewCount(), info.getDuration(),
                            Math.max(infoTime, time), info.getThumbnails().get(info.getThumbnails().size()-1).getUrl(), info.isShortFormContent(), channel);
    ...
    public static void handleNewVideo(StreamExtractor extractor, long time, Channel channel) throws Exception {
            ...
            try (StatelessSession s = DatabaseSessionFactory.createStatelessSession()) {
                if (!DatabaseHelper.doesVideoExist(s, extractor.getId())) {
                    ...
                    Video video = new Video(extractor.getId(), extractor.getName(), extractor.getViewCount(), extractor.getLength(),
                            Math.max(infoTime, time), extractor.getThumbnails().get(extractor.getThumbnails().size()-1).getUrl(), isShort, channel);
